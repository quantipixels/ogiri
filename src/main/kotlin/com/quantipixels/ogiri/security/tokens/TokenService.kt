...
private val tokenEqualityCache =
  object : LinkedHashMap<String, Boolean>(16, 0.75f, true) {
    override fun removeEldestEntry(eldest: MutableMap.MutableEntry<String, Boolean>?): Boolean = size > MAX_CACHE_SIZE
  }
...
private fun tokensMatch(
  ...
  synchronized(tokenEqualityCache) { tokenEqualityCache[key]?.let { return it } }
  val result = passwordEncoder.matches(token, tokenHash)
  synchronized(tokenEqualityCache) { tokenEqualityCache[key] = result }
  return result
}
...
open class TokenService<T : BaseToken>(
  ...
  protected open fun createTokenEntity(
    userId: Long,
    client: String,
    hashedToken: String,
    tokenType: TokenType,
    ...
    Token(
      ...
      expiryAt = expiry,
    ).apply { plainToken = plainTokenValue } as T

  @Value(
    value = "\${ogiri.auth.max-clients}",
  )
  private var maxClients: Long = 24

  @Value(
    value = "\${ogiri.auth.batch-grace-seconds}",
  )
  private var batchGraceSeconds: Long = 5

  @Value(
    value = "\${ogiri.auth.token-lifespan-days}",
  )
  private var tokenLifespanDays: Long = 14
  ...
  fun cleanOldTokens(user: TokenUser) {
    ...
    val appClients = tokens.filter { it.tokenType == TokenType.APP }.map { it.client }.toSet()
    val expectedSubClients =
      appClients.flatMap { parent -> registrations.map { it.clientIdFor(parent) } }.toSet()
    val orphanSubs =
      tokens.filter { it.tokenType == TokenType.SUB && it.client !in expectedSubClients }
    if (orphanSubs.isNotEmpty()) repository.deleteAll(orphanSubs)

    val appTokens = tokens.filter { it.tokenType == TokenType.APP }
    if (appTokens.size > maxClients) {
      ...
      val removeSubClients = remove.flatMap { parent -> registrations.map { it.clientIdFor(parent.client) } }
  ...
  fun validateSubToken(
    ...
    if (
      ...
      val token =
        getByUserIdAndClient(user.userId, authHeader.client)
          ?.takeIf { it.tokenType == TokenType.SUB && it.tokenSubtype == subTokenName }
          ?: return false
    ...
    val all =
      repository.findAllByUserId(user.userId).filter {
        it.tokenType == TokenType.SUB && it.tokenSubtype == subTokenName
      }
