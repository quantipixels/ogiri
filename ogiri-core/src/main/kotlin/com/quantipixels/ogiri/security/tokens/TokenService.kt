...
private val tokenEqualityCache =
  Caffeine.newBuilder()
    ...
    .build<String, Boolean> { key -> false }
 ...
 *
 * This service works with any token implementation extending BaseToken.
 * Subclasses can override tokenFactory() to customize token creation if needed,
 * or supply tokens directly through the public API methods.
 */
@Service
open class TokenService<T : BaseToken>(
   ...
   *
   * This method can be overridden by subclasses to provide custom token creation logic.
   * Users can extend TokenService and override this method to instantiate their custom
   * BaseToken implementations.
   *
   * Example:
   * ```kotlin
  ...
  fun isBatchRequest(
    ...
    getByUserIdAndClient(user.userId, client)
      ?.takeIf { !it.isExpired() }
      ?.updatedAt
      ?.isAfter(requestStartedAt.minusSeconds(batchGraceSeconds)) ?: false
  ...
  fun issueSubTokens(
    ...
    val registrations =
      subTokenRegistry
        .registrations()
        .filter { requestedNames?.let { names -> it.name in names } ?: it.includeByDefault }
   ...
   *
   * The returned AuthHeader should be appended to the HTTP response via [appendAuthHeaders],
   * which will set response headers: access-token, client, uid, expiry, sub-tokens
   *
   ...
   * 1. **Orphan Cleanup**: Removes sub-tokens whose parent APP tokens no longer exist
   *    (e.g., if an APP token was manually deleted but its chat/device sub-tokens remain)
   ...
   *
   * Called automatically after token creation to enforce limits. Can be called manually
   * to reclaim storage from unused tokens.
   *
   ...
   *
   * Example: If maxClients=5 and user has 7 APP tokens, the 2 oldest are deleted
   * along with all their device/chat/api sub-tokens.
   */
  @Transactional
   ...
   *
   * The grace period allows legitimate batch requests (multiple simultaneous API calls)
   * to succeed even if they arrive after token rotation has started.
   *
   ...
   *
   * Used by OgiriTokenAuthenticationFilter during authentication.
   * Does not throw exceptions; authentication failures are delegated to AuthenticationEntryPoint.
   */
  @Transactional(readOnly = true)
  fun validToken(
    plainToken: String,
  ...
  fun revokeClient(
    ...
    if (isTokenValid) {
      ...
      val subClients =
        subTokenRegistry.registrations().map { registration -> registration.clientIdFor(clientId) }
  ...
  fun renewSubToken(
    ...
    val clientId =
      authHeader.client ?: throw SecurityServiceException("error.auth.missing_token")
    val parent =
  ...
  fun tryDecodeSubBearer(encodedPart: String): SubTokenHeader? {
    return try {
      ...
      JsonCodec.mapper.readValue(json, Map::class.java)?.let { map ->
        @Suppress("UNCHECKED_CAST")
        val values = map as Map<String, Any?>
        SubTokenHeader(
  ...
  fun validateSubToken(
    ...
    if (
      ...
      val token =
        getByUserIdAndClient(user.userId, authHeader.client)
          ?.takeIf { TokenType.of(it.tokenType) == TokenType.SUB && it.tokenSubtype == subTokenName }
          ?: return false
  ...
  ): Set<String> = appClients.flatMap { parent -> registrations.map { it.clientIdFor(parent) } }.toSet()
